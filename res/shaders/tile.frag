...
float rand(vec2 p) {
    return fract(sin(dot(p ,vec2(12.9898,78.233))) * 43758.5453 * start_time);
}

vec4 getTiledTexture(sampler2D tex, vec2 uv) {

    vec2 tile_uv = floor(uv);
    
    int tile_index = int(floor(rand(tile_uv) * tile_scale * tile_scale));
    int x = tile_index % int(tile_scale);
    int y = tile_index / int(tile_scale);
    
    vec2 tile_offset = vec2(x, y) / tile_scale;
    
    vec2 local_uv = fract(uv) / tile_scale;
    
    vec2 atlas_uv = tile_offset + local_uv;
    
    return texture(tex, atlas_uv);
}
