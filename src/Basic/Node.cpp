...
Node* Node::clone(std::string instance_name, SceneGraph* new_scene_graph) {
    ...
    if (!dynamic_cast<PointLight*>(this) && !dynamic_cast<DirectionalLight*>(this)) {
        ...
        else if (dynamic_cast<LaserEmitterNode*>(this)) {
            LaserEmitterNode* p = dynamic_cast<LaserEmitterNode*>(this);
            copy = new LaserEmitterNode(pModel, instance_name + "_" + this->name, p->startRay);
        }
        else {
...
void LaserEmitterNode::setBuffer()
{
...
LaserEmitterNode::LaserEmitterNode(std::shared_ptr<Model> model, std::string nameOfNode, Ray start_ray) : Node(model, nameOfNode)
{
    startRay = start_ray;
    startRay.direction = glm::normalize(startRay.direction);
    startRay.length = 100.f;
    startRay.origin = transform.getGlobalPosition();
    //rays.push_back(start_ray);
    std::vector<glm::vec3> v = { startRay.origin, startRay.origin + startRay.direction * startRay.length };
    addRays(v);
    setBuffer();
}

void LaserEmitterNode::addRays(std::vector<glm::vec3>& rays)
{
    vertices.clear();

    if (rays.size() > 1) {
        vertices.push_back(rays[0].x);
        vertices.push_back(rays[0].y);
        vertices.push_back(rays[0].z);
        for (int i = 0; i < rays.size(); ++i) {

            vertices.push_back(rays[i].x);
            vertices.push_back(rays[i].y); //koniec poprzedniej
            vertices.push_back(rays[i].z);
...
void LaserEmitterNode::forceUpdateSelfAndChild()
{
    if (parent) {
        transform.computeModelMatrix(parent->transform.getModelMatrix());
    }
...
void LaserEmitterNode::drawSelfAndChild()
{
    if (pModel && is_visible && in_frustrum) {
