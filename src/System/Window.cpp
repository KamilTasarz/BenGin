...
void mouseCallback(GLFWwindow* window, double posX, double posY) {
	...
	if (ServiceLocator::getWindow()->firstMouseMovement) {
		...
		ServiceLocator::getWindow()->firstMouseMovement = false;
	}

	float offsetX = x - ServiceLocator::getWindow()->lastX;
	float offsetY = ServiceLocator::getWindow()->lastY - y; // reversed
...
std::unordered_map<InputKey, InputDeviceState> Window::getGamepadState(int joystick_id)
{
